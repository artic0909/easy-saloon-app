...
class StaffCompleteBookingsPage extends StatefulWidget {
  final bool embed;
  const StaffCompleteBookingsPage({super.key, this.embed = false});

  @override
  State<StaffCompleteBookingsPage> createState() => _StaffCompleteBookingsPageState();
}

class _StaffCompleteBookingsPageState extends State<StaffCompleteBookingsPage> {
  ...
  Future<void> _fetchCompletedBookings() async {
    ...
    try {
      final response = await _apiService.dio.get('/staff/completed-bookings', queryParameters: {
      ...
      Get.snackbar("Error", "Failed to load completed tasks",
  ...
  Widget _buildEmptyState() {
    return Center(
      child: ListView(
        ...
        children: [
          Icon(Icons.check_circle_outline, size: 64.w, color: Colors.white24),
          SizedBox(height: 16.h),
          Text(
            "No Completed Tasks",
          ...
          SizedBox(height: 8.h),
          Text(
            "Completed task logs will appear here.",
            textAlign: TextAlign.center,
            style: TextStyle(color: Colors.white38, fontSize: 12.sp),
          ),
        ],
