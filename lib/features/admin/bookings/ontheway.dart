...
class AdminOnTheWayBookingsPage extends StatefulWidget {
  const AdminOnTheWayBookingsPage({super.key});

  @override
  State<AdminOnTheWayBookingsPage> createState() => _AdminOnTheWayBookingsPageState();
}

class _AdminOnTheWayBookingsPageState extends State<AdminOnTheWayBookingsPage> {
  ...
  Future<void> _fetchBookings() async {
    ...
    try {
      final response = await _apiService.dio.get('/admin/pending-bookings', queryParameters: {
      ...
      if (response.data['status'] == 'success') {
        ...
        setState(() {
          ...
          // Filter to show only on_the_way and started bookings
          _bookings = (rawData as List).where((b) {
            final status = b['status']?.toString().toLowerCase() ?? '';
            return status == 'on_the_way' || status == 'started';
          }).toList();
          _isLoading = false;
        });
      }
      ...
      Get.snackbar("Error", "Failed to load on-the-way bookings",
  ...
  Widget _buildEmptyState() {
    return Center(
      child: ListView(
        ...
        children: [
          Icon(Icons.directions_car, size: 64.w, color: Colors.white24),
          SizedBox(height: 16.h),
          Text(
            "No Bookings On the Way",
            textAlign: TextAlign.center,
            style: TextStyle(fontFamily: 'Playfair Display', fontSize: 18.sp, color: Colors.white, fontWeight: FontWeight.bold),
          ),
          SizedBox(height: 8.h),
          Text(
            "No staff is currently en route to a booking.",
            textAlign: TextAlign.center,
            style: TextStyle(color: Colors.white38, fontSize: 12.sp),
          ),
        ],
